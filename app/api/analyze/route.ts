...
async function saveToSupabase(url: string, tagCounts: Record<string, number>) {
  try {
    ...
    console.log("Dados a serem salvos:", { url, tagCounts })

    // Usar withRetry para tentar a operação várias vezes se necessário
    const { data, error } = await withRetry<{ data: Array<{ id: number }> | null; error: any }>(async () => {
    ...
    console.error("Erro detalhado ao salvar no banco de dados:", {
      error,
